...
const Leave = () => {
  ...
  const leaveList = async loc => {
    ...
    try {
      ...
      if (leaveTypesData.length > 0) {
        setLeaveType(leaveTypesData[0].value); // Set default leave type
      }
  ...
  const LeaveSelected = async item => {
    ...
    await fetch(`${clientUrl}api/leavebalance`, requestOptions)
      .then(response => response.json())
      .then(result => {
        console.log('left ', result.Leave_balance);
      ...
      <ScrollView
        ...
        <Text style={styles.dateText}>To Date: {toDate.toDateString()}</Text>
          ...
          <Picker
            ...
            style={styles.picker}
            onValueChange={itemValue => LeaveSelected(itemValue)}>
